...
- **Network:** only `https://api.coinset.org` and `https://testnet11.api.coinset.org`, the public full-node API the View, The Wall and My Memos tabs read from, plus `https://api.xchandles.com` and `https://api.namesdao.org`, to turn an `@name` typed in Generate's receive address or in View into an address.
...
- **`storage.persistent_webview`:** lets the app keep its data between launches: display settings and the lists of inscriptions it has already indexed for The Wall, My Memos and addresses looked up in View. Without it Sage runs apps in incognito mode, so every launch would re-scan the chain from scratch. Nothing is stored anywhere but your device.
...
- **Persistent storage:** the manifest requests `storage.persistent_webview`, so the app keeps its display settings and its indexes of The Wall, My Memos and addresses looked up in View between launches; in incognito mode Sage wipes them at every close and each launch re-scans the chain. It also worked around a Sage 0.13.0 bug on Windows, where the "incognito storage" sandbox test failed and blocked every app running in incognito mode ([xch-dev/sage#841](https://github.com/xch-dev/sage/pull/841), released in 0.13.1).

## Running the pages outside Sage

Unzip the package and open `index.html` (Home), `generator.html`, `viewer.html`, `feed.html` or `myfeed.html` in a
